...
<p><span>{{__('partner.service_or_activity_name')}}:</span>
	@foreach ($details->service as $service)
		<p>{{__('partner.name')}}: {{$service['name']}}, {{__('service.description')}}: {{$service['description']}}<span class="coma">;</br></span><p>
	@endforeach
</p>
...
<p><span>{{__('partner.articles')}}:</span>
	@if ($details->article == 'art_d') 
		<a href="{{asset('storage/articles/'.$details->art_d)}}" target="blank">{{$details->art_d}}</a>
