        ...
        <div class="row justify-content-center">
            <div class="col-xl-3 col-lg-3 col-md-3 col-sm-12">
                <ul>
                    ...
                    <li><a href="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/aboutus')}}">{{__('main.aboutus_page')}}</a></li>
                    <li><a href="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/partner')}}">{{__('main.become_partner_page')}}</a></li>
                ...
                @foreach(array_chunk($footerCategories->toArray(), 3) as $k => $data)
                    <ul class="bubbles">
                        @foreach($data as $item)
                            <li><a href="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/' . __('urls.listing') . '/' . $item['lang']['slug'])}}">{{$item['lang']['name']}}</a></li>
                        @endforeach
                    </ul>
                @endforeach
            </div>
