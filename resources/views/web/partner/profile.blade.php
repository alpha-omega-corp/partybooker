<div class="tab" tab="profile" style="display: block">
	...
	<div class="profile-info">
		...
		@if($user->partnerInfo->currentPlan && !in_array(strtolower($user->partnerInfo->currentPlan->name), ['basic', 'commission']))
			@foreach($adverts as $k =>  $advert)
				@if($advert->status == \App\Models\Advert::STATUS_DRAFT)
					<ul class="serviceDetails{{$k+1}} attention">
						<li block="serviceDetails{{$k+1}}"><h4>{{__('partner.service_details')}} #{{$k+1}}:  </h4> {{__('service.for')}} {{__('service.'.$advert->view_name)}}</li>
				...
				@else
					@include('web.partner.advert.details.' . $advert->view_name , ['iterator' => $k + 1, 'advert' => $advert])
				@endif
			@endforeach
		@endif
