<ul class="serviceDetails{{$iterator}}">
	...
	<li class="li"><span>{{__('partner.opening_upon_appointment')}}:</span> {{$advert->service->opening_upon}}</li>
...
{{--	<li class="li"><span>{{__('partner.price')}}:</span> {{$user->partnerInfo->price}} @if($advert->service->price_for == 'other') {{$advert->service->other_price}} @else {{__('partner.'.$advert->service->price_for)}} @endif</li>--}}
	...
	<li class="li">
		@foreach ($advert->service->service ?? [] as $service)
			{{__('partner.name')}}: {{$service['name']}}, {{__('service.description')}}: {{$service['description']}}<span class="coma"></br></span>
		@endforeach
	</li>
	...
	<li class="li">
		@if ($advert->service->article == 'art_d') 
			<a href="{{asset('storage/articles/'.$advert->service->art_d)}}" target="blank">{{$advert->service->art_d}}</a>
	...
	<li class="li"><h6>{{__('partner.food_services_info')}}</h6></li>
	<li class="li"><span>{{__('partner.food_on_site')}}:</span> {{$advert->service->food_on_site}}</li>
	<li class="li">{{$advert->service->yes_food}}</li>
	<li class="li"><span>{{__('partner.catering_service_available')}}:</span> {{$advert->service->catering}}</li>
	...
	@if (strlen($advert->service->caterers_aff) > 0)
		<li class="li"><span>{{__('partner.affiliated_partybooker_caterer')}}:</span> {{$advert->service->caterers_aff}}</li>
	@endif
	<li class="li food"><a href="#" class="button" data-db="{{$iterator}}">{{__('partner.edit')}}</a></li>
