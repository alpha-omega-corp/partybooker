...
<div class="partner-details-form popup popup-form partnerdetails popup-form-xl">
	@if (Auth::user()->type == 'admin')
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/cp/adverts/'.$advert->id.'/equipment')}}" method="POST" enctype="multipart/form-data">
			<input type="text" name="id_partner" value="{{$user->id_partner}}" hidden />
	@else
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/adverts/'.$advert->id.'/equipment')}}" method="POST" enctype="multipart/form-data">
	@endif
		{{ csrf_field() }}
		<div class="close"></div>
				...
				<div class="row">
					...
					<div class="col-6">
						...
						<select name="price_for" required >
							...
							<option value="space_rental_price_per_day">{{__('partner.space_rental_price_per_day')}}</option>
