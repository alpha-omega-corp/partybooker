<div class="partner-details-form popup popup-form partnerdetails popup-form-xl">
	@if (Auth::user()->type == 'admin')
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/cp/adverts/'.$advert->id.'/wine-lovers')}}" method="POST" enctype="multipart/form-data">
			<input type="text" name="id_partner" value="{{$user->id_partner}}" hidden />
	@else
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/adverts/'.$advert->id.'/wine-lovers')}}" method="POST" enctype="multipart/form-data">
	@endif
		{{ csrf_field() }}
		<div class="close"></div>
		...
		<div class="row mt-10">
				...
				<div class="row">
					...
					<div class="col-6">
						...
						<select name="price_for" required >
							...
							<option value="other" class="other-price">{{__('become_partner.other')}}</option>
						</select>
			...
			<div class="col-lg-5 col-md-6 payment-options">
				...
				@foreach(\App\Helpers\PaymentMethodsTranslatorHelper::$_methods as $i)
					<span class="checkbox-item wide">
						<input type="checkbox" name="pay_methods[]" value="{{$i}}" required >
						<span>{{\App\Helpers\PaymentMethodsTranslatorHelper::translate($i)}}</span>
					</span>
				@endforeach
				<span class="checkbox-item wide">
				...
				<div class="set">
					<div class="set-service_activity">
						...
						<div class="service-activity">
							<input type="text" name="service_activity_name1" maxlength="30" placeholder="Name" required />
							<textarea name="service_activity_description1" rows="2" maxlength="300" placeholder="Description" class="mt-10" required /></textarea>
						</div>
					</div>
				...
				<div class="set">
					<div class="set-participant">
						...
						<div class="participant">
							<div class="row">
								<div class="col-sm-6">
									<input type="text" name="space_name1" maxlength="30" placeholder="{{__('partner.space_name')}}" required />
								</div>
								<div class="col-sm-3">
									<input type="number" name="min_num1" min="0" placeholder="min" required />
								</div>
								<div class="col-sm-3">
									<input type="number" name="max_num1" min="1" placeholder="max" required />
								</div>
							</div>
						</div>
					</div>
				...
				<div class="set">
					<div class="set-wine">
						...
						<div class="wine">
							...
							<textarea name="wine_description1" rows="2" maxlength="300" placeholder="Grape variety" class="mt-10" /></textarea>
						</div>
					</div>
			...
			<div class="col-lg-5 offset-lg-1 col-md-6">
				...
				<div class="set">
					<div class="set-reward">
						...
						<div class="reward">
							<input type="text" name="price_name1" placeholder="{{__('partner.wine_name')}}" required />
							<input type="text" name="contest_name1" placeholder="{{__('partner.contest_name')}}" required class="mt-10"/>
							<span class="radio-item">
			...
			<div class="col-lg-5 col-md-6">
				...
				<div class="for-article-file additional-field mt-10">
					<label for="article" class="label">
						...
						<input type="file" id="article" name="article_d"  class="articleFile imagesFileUpload" accept="image/*, .pdf" />
					</label>
		...
		<div class="row mt-10">
			<div class="offset-lg-1 col-lg-5 col-md-6 furniture-options">
				...
				@endforeach
				<span class="checkbox-item wide">
					<input type="checkbox" name="furniture_eq[]" class="other-field" field="furniture_eq" value="other" />
					<span>{{__('become_partner.other')}}</span>
				</span>
