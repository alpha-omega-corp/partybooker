...
<div class="partner-details-form popup popup-form partnerdetails popup-form-xl">
	@if (Auth::user()->type == 'admin')
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/cp/adverts/'.$advert->id.'/entertainment')}}" method="POST">
			<input type="text" name="id_partner" value="{{$user->id_partner}}" hidden />
	@else
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/adverts/'.$advert->id.'/entertainment')}}" method="POST">
	@endif
		{{ csrf_field() }}
		<div class="close"></div>
