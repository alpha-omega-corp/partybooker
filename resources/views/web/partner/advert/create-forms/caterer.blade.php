<div class="partner-details-form popup popup-form partnerdetails popup-form-xl">
	@if (Auth::user()->type == 'admin')
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/cp/adverts/'.$advert->id.'/caterer')}}" method="POST">
		<input type="text" name="id_partner" value="{{$user->id_partner}}" hidden />
	@else
		<form class="reg container" action="{{url(App\Http\Middleware\LocaleMiddleware::getLocale().'/adverts/'.$advert->id.'/caterer')}}" method="POST">
	@endif
		...
		<div class="row">
			<div class="col-lg-3 offset-lg-1 col-md-4">
				...
				@foreach(\App\Helpers\LogisticsTranslatorHelper::$_methods as $key)
					<span class="checkbox-item wide">
						<input type="checkbox" name="logistic[]" value="{{$key}}" />
						<span>{{\App\Helpers\LogisticsTranslatorHelper::translate($key)}}</span>
					</span>
				@endforeach
				
				<label class="mt-10">{{__('partner.staff')}}</label>
				@foreach(\App\Helpers\StaffTranslatorHelper::$_methods as $key)
					<span class="checkbox-item wide">
						<input type="checkbox" name="staff[]" value="{{$key}}" />
						<span>{{\App\Helpers\StaffTranslatorHelper::translate($key)}}</span>
					</span>
				@endforeach
