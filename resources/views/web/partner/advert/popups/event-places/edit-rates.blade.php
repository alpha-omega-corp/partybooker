    ...
    <div class="row">
        ...
        <div class="col-6">
            <select name="price_for" required>
                <option @if($advert->service->price_for == 'space_rental_price_per_hour') selected
                        @endif value="space_rental_price_per_hour">{{__('partner.space_rental_price_per_hour')}}</option>
                <option @if($advert->service->price_for == 'fixed_price_per_person') selected
                        @endif value="fixed_price_per_person">{{__('partner.fixed_price_per_person')}}</option>
                <option @if($advert->service->price_for == 'flat_rate_for_service') selected
                        @endif value="flat_rate_for_service">{{__('partner.flat_rate_for_service')}}</option>
                <option @if($advert->service->price_for == 'other') selected @endif value="other"
            ...
            @if($advert->service->price_for == 'other')
                <div class="for-price additional-field mt-10 visible">
                    <input type="text" name="other_price" maxlength="35" class="add-input-price"
                           value="{{$advert->service->other_price}}"/>
                </div>
            @else
