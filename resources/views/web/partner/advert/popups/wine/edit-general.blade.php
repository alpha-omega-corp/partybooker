...
<x-dashboard.modal
    ...
    <div class="set">
        <div class="set-service_activity">
            <input type="text" value="{{count($advert->service->service)}}" name="set_num_service"
                   hidden/>
            <div class="service-activity">
                <input type="text" name="service_activity_name1" maxlength="30" placeholder="Name"
                       value="{{isset($advert->service->service[0]) ? $advert->service->service[0]['name'] : ''}}"
                       required/>
                <textarea name="service_activity_description1" rows="2" maxlength="300"
                          placeholder="Description" class="mt-10"
                          required/>{{isset($advert->service->service[0]) ? $advert->service->service[0]['description'] : '' }}</textarea>
            </div>
            @for ($i = 1; $i<count($advert->service->service); $i++)
                <div class="service-activity mt-10">
                    <input type="text" name="service_activity_name{{$i+1}}" maxlength="30"
                           placeholder="Name"
                           value="{{isset($advert->service->service[$i]) ? $advert->service->service[$i]['name'] : ''}}"
                           required/>
                    <textarea name="service_activity_description{{$i+1}}" rows="2" maxlength="300"
                              placeholder="Description" class="mt-10"
                              required/>{{isset($advert->service->service[$i]) ? $advert->service->service[$i]['description'] : ''}}</textarea>
                </div>
            @endfor
        </div>
    ...
    <div class="set">
        <div class="set-participant">
            <input type="text" value="{{sizeof(json_decode($advert->service->participant))}}"
                   name="set_num_participant" hidden/>
    ...
    <div class="set">
        <div class="set-aff">
            <input type="text" value="{{sizeof(json_decode($advert->service->affiliation))}}"
                   name="set_num_aff" hidden/>
            @if(isset(json_decode($advert->service->affiliation)[0]))
                <div class="affiliations">
                    <input type="text" name="aff_name1" maxlength="30" placeholder="Name"
                           value="{{substr(json_decode($advert->service->affiliation)[0], 5, strpos(json_decode($advert->service->affiliation)[0], 'link:') - 7)}}"/>
                    <input type="url" name="aff_link1" placeholder="Link" class="mt-10"
                           value="{{substr(json_decode($advert->service->affiliation)[0],strpos(json_decode($advert->service->affiliation)[0], 'link:')+5, strlen(json_decode($advert->service->affiliation)[0]))}}"/>
                </div>
            @else
                <div class="affiliations">
