<div class="tab" tab="main" style="display: block">
	...
	@if(\Auth::user()->type == 'admin')
		<div class="admin">
			...
			<div class="row">
				<div class="col-md-3">
					<a href="#" data-toggle="modal" data-target="#exampleModal" style="font-size: 13px; line-height: normal" class="button" data-id="{{$user->partnerInfo->id}}">{{__('partybooker-cp.set_discount_amount')}}</a>
				</div>
				@if($user->partnerInfo->currentPlan)
					<div class="col-md-3">
						<a href="#" data-toggle="modal" id="add-category-button" data-target="#add-category" style="font-size: 13px; line-height: auto" class="button" data-id="{{$user->partnerInfo->id}}">{{__('partybooker-cp.add_sub_category')}}</a>
					</div>
				@endif
			</div>
					...
					<div class="modal-content">
						<div class="modal-header">
							<h5 class="modal-title"
							    id="exampleModalLabel">{{__('partybooker-cp.add_sub_category')}}</h5>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
	...
	<div class="status">
			...
			@else
				@if ($user->partnerInfo->public == 0)
					...
					@if(in_array($user->partnerInfo->plan, ['basic', 'commission']))
						<a href="#" class="button" data-id="{{$user->partnerInfo->id_partner}}">{{__('partner.publish')}}</a>
				...
				@else
					...
					<a href="#" class="button" data-id="{{$user->partnerInfo->id_partner}}">{{__('partner.draft')}}</a>
				@endif
			@endif
		</div>
		...
		<ul>
			...
			@if ($user->partnerInfo->payment_status == 0)
				<li class="topay"><a href="{{url(\App\Http\Middleware\LocaleMiddleware::getLocale() . ((\Auth::user()->type == 'admin') ? '/cp' : '') . '/partner-cp/' . $user->id_partner . '/plans')}}" class="button">{{__('partner.make_payment')}}</a></li>
			@endif
		...
		@if($user->partnerInfo->vipPlan)
			<ul>
				...
				<li><span>{{__('partner.payment')}}: </span>
					@if(!$user->partnerInfo->vipPlan->is_payed || ($user->partnerInfo->vipPlan->end_date && date("Y-m-d") > $user->partnerInfo->vipPlan->end_date))
						N/A
					@else
	...
	<div class="statistics">
		...
		<ul>
			...
			<li><span>{{__('partner.website_clicks')}}: </span>{{$user->partnerInfo->statistic->website ?? 0}}</li>
			<li><span>{{__('partner.address_clicks')}}: </span>{{$user->partnerInfo->statistic->address ?? 0}}</li>
			<li><span>{{__('partner.direct_requests')}}: </span>{{$user->partnerInfo->statistic->direct ?? 0}}</li>
	...
	<div class="rating-stat">
		...
		<ul class="stars">
			@for($i = 1; $i <= 5; $i++)
				...
				@endif
			@endfor
			<li class="pointed">{{__('partner.based_on')}} {{$user->partnerInfo->votes}} {{__('partner.rates')}}</li>
		</ul>
		...
		<div class="rated">
			@for($i = 5; $i > 0; $i--)
				<ul class="stars">
					...
					@endfor
					<li class="pointed">{{isset($user->partnerInfo->rateGroup[$i]) ? $user->partnerInfo->rateGroup[$i] : 0}} {{__('partner.rates')}}</li>
				</ul>
			@endfor
		</div>
	</div>
