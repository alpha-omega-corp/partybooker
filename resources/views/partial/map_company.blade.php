<div>
	<input type="text" maxlength="128" id="map_address" name="map[address]" placeholder="{{__('become_partner.location')}}" value="{{$info[0]->address}}" autocomplete="off"/>
	<input type="hidden" id="map_lat" name="map[lat]" value="{{$info[0]->lat ? $info[0]->lat : '' }}"/>
	<input type="hidden" id="map_lon" name="map[lon]" value="{{$info[0]->lat ? $info[0]->lat : '' }}"/>
...
@push('footer')
	...
	<script type="text/javascript">
		...
		$("#map").locationpicker({
			location: {latitude: '{{$info[0]->lat ? $info[0]->lat : '' }}', longitude: '{{$info[0]->lon ? $info[0]->lon : "" }}'},
			radius: 0,
			zoom: 14,
			scrollwheel: true,
			inputBinding: {
