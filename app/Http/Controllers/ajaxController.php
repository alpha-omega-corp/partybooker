...
class ajaxController extends Controller
{
	...
	public function rate(Request $request) {
		...
		$chek_email = false;
		$rate = DB::table('rates')->where('id_partner',$id_partner)->get();
		foreach ($rate as $rate) {
			if ($rate->user_email == 'simple@simple.com') {
				$chek_email = true;
				break;
			} 
		}
		if ($chek_email == false) {
			...
			if($lang == 'en') {
				...
				$msg = "Je vous remercie!";
			};
		
			$rating = DB::table('rates')->where('id_partner', $id_partner)->get();
			$rated = sizeof($rating);
			$total = 0;
			...
			if($lang == 'en') {
				$msg = "You have rated this service";
			} else {
				$msg = "Vous avez évalué ce service!";
			};
			$average_rate = DB::table('partners_info')->
				where('id_partner', $id_partner)->
				value('average_rate');
			return response()->json(array('msg' => $msg, 'rate' => $average_rate));
		}
