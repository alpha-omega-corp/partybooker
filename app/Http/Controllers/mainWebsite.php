...
class mainWebsite extends Controller
{
	...
	public function home() {
		...
		//				where('public', 1)->
		//				whereHas('currentPlan', function ($q){
		//					$q->where('name', 'LIKE', '%Exclusif%');
		//				})->
		//				get();
	...
	public function partner() {
		$plans = Plans::with('planOptions')->get();
		foreach($plans as $plan) {
			$plan->name = strtolower($plan->name);
			$plan->options = $this->getPlanOptions($plan->planOptions);
		}
		return view('partner', ['plans'=>$plans]);
    }
	...
	public function faq() {
		$top = DB::table('partners_info')->
						where('payment_status', 1)->
						where('public', 1)->
						where('plan', 'exclusif')->get();
